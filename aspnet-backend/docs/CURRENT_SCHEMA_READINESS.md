...
<!-- doc-consistency: SCHEMA_CURRENT_RUNTIME_MIGRATIONS=186 -->
...
| EF migration classes | 186 | Source classes in the current tree, recounted 2026-08-22 (excludes Designer and snapshot files). Latest: `20260822082641_AddReviewTransactionLink` — `reviews.TransactionId`, a unique index on `(TenantId, ReviewerId, TransactionId)` filtered to non-null, and removal of the older unique index on `(TenantId, ReviewerId, TargetUserId)`, for ledger row 4.29; the previous tail was `20260821164404_AddExchangeTwoPartyConfirmation`. 🔴 The removed index was **stricter than Laravel**, not equivalent to it: Laravel's rule is one review per reviewer per *transaction* (`ReviewService::create`, `app/Services/ReviewService.php:428-451`), so two members who complete a second exchange together may legitimately review each other again, and the old index rejected that at the database with no message any client could show. 🔴 This read 185 with a 2026-08-21 tail until 2026-08-22; `check-doc-scores.mjs` counts the tree and fails on any drift, so the number cannot go stale silently. |
| Runtime-discovered migration IDs | 184 | Applicable chain from `InitialCreate` through `AddReviewTransactionLink`. **Measured 2026-08-22** with `dotnet ef migrations list` and a full `dotnet ef database update` against a disposable PostgreSQL 16.4 container on port 15433 (never the local Laravel database, which is a production-derived snapshot): 184 applied, 184 rows in `__EFMigrationsHistory`, `max(MigrationId) = 20260822082641_AddReviewTransactionLink`, and `has-pending-model-changes` reported "No changes have been made to the model since the last migration." A **populated** 183→184 replay was also run rather than a blank one only: an existing review row survived the index drop and column add with `TransactionId` NULL, a second review of the same transaction was rejected by the new unique index, and a second review of the same *person* against a *different* transaction was accepted — which the old index would have refused. The relationship is `classes − quarantined = runtime IDs` (186 − 2 = 184). The `SCHEMA_CURRENT_RUNTIME_MIGRATIONS` marker above is 186 because `check-doc-scores.mjs` counts FILES ON DISK despite its name; do not "reconcile" the two by editing either to match the other. 🔴 Was 163 on 2026-08-14; twenty-two migrations landed between 2026-08-14 and 2026-08-22 (partner venues, support actions, account relationships, supporter message-view audits, authority attestations, platform capability overrides, revoked tokens, volunteer donation/project fields, skill categories, volunteer opportunity remote/coordinates, exchange two-party confirmation, review/transaction link). |
