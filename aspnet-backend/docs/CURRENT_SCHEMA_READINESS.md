...
<!-- doc-consistency: SCHEMA_CURRENT_RUNTIME_MIGRATIONS=187 -->
...
| EF migration classes | 187 | Source classes in the current tree, recounted 2026-08-24 (excludes Designer and snapshot files). Latest: `20260824114432_AddListingHoursAndServiceType`, which adds nullable `listings.HoursAvailable` and `listings.ServiceType` for journey-ledger row 1.20; the previous tail was `20260822082641_AddReviewTransactionLink`. `check-doc-scores.mjs` counts the tree and fails on any drift, so the number cannot go stale silently. |
| Runtime-discovered migration IDs | 185 | Applicable chain from `InitialCreate` through `AddListingHoursAndServiceType`. **Measured 2026-08-24** by the fresh migrated-database listing-management tests and the full 3,897-test API aggregate; `has-pending-model-changes` reported "No changes have been made to the model since the last migration." The relationship is `classes − quarantined = runtime IDs` (187 − 2 = 185). The `SCHEMA_CURRENT_RUNTIME_MIGRATIONS` marker above is 187 because `check-doc-scores.mjs` counts FILES ON DISK despite its name; do not "reconcile" the two by editing either to match the other. The latest migration is additive and nullable, so existing listing rows retain their prior data while the React edit journey gains durable hours-cap and delivery-mode storage. |
