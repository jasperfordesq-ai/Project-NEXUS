...
<!-- doc-consistency: SCHEMA_CURRENT_RUNTIME_MIGRATIONS=188 -->
...
| EF migration classes | 188 | Source classes in the current tree, recounted 2026-08-29 (excludes Designer and snapshot files). Latest: `20260829113015_CompleteCaringCaregiverLinkLifecycle`, which adds the caregiver-link consent lifecycle (recipient confirmation, consent evidence and verifier, approval and rejection audit fields, and the `rejected` state) mirroring the Laravel migration of the same name. The previous tail was `20260824114432_AddListingHoursAndServiceType`, which adds nullable `listings.HoursAvailable` and `listings.ServiceType` for journey-ledger row 1.20; the previous tail was `20260822082641_AddReviewTransactionLink`. `check-doc-scores.mjs` counts the tree and fails on any drift, so the number cannot go stale silently. |
| Runtime-discovered migration IDs | 186 | Applicable chain from `InitialCreate` through `CompleteCaringCaregiverLinkLifecycle`. **Measured 2026-08-24** by the fresh migrated-database listing-management tests and the full 3,897-test API aggregate; `has-pending-model-changes` reported "No changes have been made to the model since the last migration." The relationship is `classes − quarantined = runtime IDs` (188 − 2 = 186). The `SCHEMA_CURRENT_RUNTIME_MIGRATIONS` marker above is 188 because `check-doc-scores.mjs` counts FILES ON DISK despite its name; do not "reconcile" the two by editing either to match the other. The latest migration is additive and nullable, so existing listing rows retain their prior data while the React edit journey gains durable hours-cap and delivery-mode storage. |
