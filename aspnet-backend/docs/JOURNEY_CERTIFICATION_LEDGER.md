...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 — Core member journeys (React) | 42 | 0 | 19 | 0 | 3 | 20 | 0.293 |
...
| 1.21 | Exchange: request → accept → complete → credits move | BROKEN | **The named cause has CHANGED. Settlement now works; the exchange DETAIL RESPONSE is the new blocker.** 🔴 Do not read this row as "still no settlement" — that was the cause until 2026-08-21 and it is fixed. Migration **`20260821164404_AddExchangeTwoPartyConfirmation`** added the five columns Laravel records two-party confirmation in (`requester_confirmed_at/hours`, `provider_confirmed_at/hours`, `final_hours` — names, nullability and `decimal(5,2)` precision copied from `database/schema/mysql-schema.sql:8909-8917`, table `exchange_requests`), and `ExchangeService.ConfirmHoursAsync` settles on `PersonalWalletLedgerService` the way `GroupExchangeService.CompleteAsync` does: one transaction, advisory locks on both members in sorted order, balance derived from the ledger, no balance column written. `POST /complete` is now Laravel's `markReadyForConfirmation` (provider-only, `in_progress → pending_confirmation`, moves no credits) and `POST /confirm` records one party's clamped figure and settles only when both are present and agree within 0.25h — wider apart goes to Disputed and moves nothing. **MEASURED, on real PostgreSQL, in `ExchangeSettlementTests` (10 tests): credits move exactly −2.00/+2.00 with both ledger legs on one `exchange` transaction row; the mean settles inside tolerance (1.90h); outside tolerance disputes with no movement; a one-sided confirmation records and moves nothing; a non-participant gets 404; a replay after settlement is refused and does not pay twice; two simultaneous confirmations settle exactly once (advisory-lock race); an unaffordable settlement returns 422 and keeps the counterparty's earlier confirmation.** **WHY STILL BROKEN, and this is a NEW confirmed defect, not the old one: the unchanged React client cannot render the exchange at all.** `ExchangeDetailPage.tsx:172-196` does `api.get<Exchange>('/v2/exchanges/{id}')` and then `setConfirmHours(response.data.proposed_hours.toString())`. ASP.NET's `ExchangesController.GetExchange` returns a BARE object (no `{success,data}` envelope) whose fields are `initiator`/`listing_owner`/`agreed_hours`, and whose `status` serialises as `requested` / `inprogress` / `pendingconfirmation` from `ExchangeStatus.ToString().ToLowerInvariant()`. The client reads `requester_id`, `provider_id`, `proposed_hours` and the statuses `pending_provider` / `in_progress` / `pending_confirmation` (`types/api.ts:1655-1673`, `lib/exchange-status.ts:27-51`, `ExchangeDetailPage.tsx:221-222, 396-404`), so `proposed_hours` is `undefined`, `.toString()` throws, the catch renders `detail.not_found`, and every action button is hidden because `isProvider` is false. Nothing else serves `GET /api/v2/exchanges/{id}` — grep confirms. **This was invisible before** because the journey died earlier, at settlement. **Unmet conditions: 1 (client cannot drive it) and 3 (no Laravel control in the same run). Conditions 2, 4 and 5 are met at the API boundary.** Next action, in order: an Implementer maps the exchange read/write responses to Laravel's envelope, party field names and status vocabulary — additively, in ASP.NET only, never by touching the client — then re-run the smoke step `journey-exchange-request-accept-complete` (which already drives both parties' confirmations through the browser) against a rebuilt dev image, with the Laravel control arm. **Earlier history, still true:** the Laravel control arm passes the whole journey and the credits move (exchange 63, balances requester 99→98 / provider 26→27, settlement visible in wallet history). Four defects fixed on 2026-08-21: (a) `GET /v2/exchanges/config` was a do-nothing stub returning four fields no client reads and omitting all four the client does, which took the feature off the map because a missing boolean reads as `false`; (b) the client sends POST while the controller declared only PUT, so accept hit an `AdminOnly` empty-array stub and answered the listing owner 403; (c) three guard-free aliases mutated status with no participant and no transition check — `POST /confirm` was measured live reopening a COMPLETED exchange that already carried a settled `TransactionId`; (d) `POST /v2/exchanges/{id}/rate` returned HTTP 500 `AmbiguousMatchException` and `GET .../ratings` was a stub shadowing the real read. Condition 5 clean for all 14 endpoints on the path; stub baseline still 316. Guarded by `ExchangeJourneyContractTests` + `ExchangeSettlementTests` + `ExchangeConcurrencyTests` + the smoke step. 🔴 **UPDATE 2026-08-21 (later): the response-shape blocker described above is FIXED, and fixing it exposed TWO MORE that nothing had named.** New `Support/Exchanges/ExchangeContractMapper.cs` is now the single projection and the single status vocabulary; `WireStatus` THROWS on an unmapped enum member rather than lowercasing a PascalCase name onto the wire, so a new status fails a test instead of shipping. Every endpoint now returns Laravel's `{data, meta}` envelope with `requester*`/`provider*`/`proposed_hours` and Laravel's status spellings. The two newly found blockers: (e) `GET /v2/exchanges/check` returned a bare `{can_exchange, reason}` where the client reads `data` — which broke EVERY listing page, so **the Request Exchange button was missing from every listing in every community and the feature had no entry point at all**; and (f) `POST /v2/exchanges` bound only `agreed_hours`, so **the hours a member typed were silently discarded**. Also fixed: `?status=active` was swallowed by `Enum.TryParse` and returned the UNFILTERED list (an unknown filter now 400s rather than lying); `needs-attention-count` omitted `PendingConfirmation`, making the confirm action unreachable dead code; and decline wrote `declined`, a status Laravel does not have (it writes `cancelled`). Contract taken from Laravel's hand-written `formatExchange` (`ExchangesController.php:384-419`) and cross-checked against a SECOND unchanged client (`web-uk/src/routes/exchanges.js:111-175`) — both agree. 27 new cases in `ExchangeClientContractTests`, one driven off `Enum.GetValues` so a new status cannot ship unmapped, and one enumerating every field `ExchangeDetailPage.tsx` reads rather than guessing. Condition 5 clean on all 12 endpoints AND every method body on the path was opened by hand, because the stub scanner is blind to a do-nothing method carrying many routes. Status stays BROKEN in this commit only because promoting it is a banking transaction and needs CI green at a pushed SHA first; on the evidence it is PARTIAL — ADR-0004 conditions 2, 4 and 5 are met, and 1 and 3 are unmet for one reason: **no browser run**, because the shared dev container still serves a pre-fix image. Remaining known shortfalls, all recorded rather than hidden: `prep_time` is accepted and dropped (no column — needs a migration); `status_history` is DERIVED from timestamps, so a member sees a shorter timeline than on Laravel (no `exchange_history` table here); `broker_notes`/`broker_id`/`risk_level` are honest nulls (the local `broker_notes` rows carry `is_private`, so surfacing them would risk leaking a staff note); and `needs-attention-count` still counts `accepted`, which Laravel excludes — subtractive, so it needs its own evidence. |
