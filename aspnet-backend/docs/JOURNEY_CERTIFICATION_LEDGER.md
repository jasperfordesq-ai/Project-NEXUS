...
| 1.21 | Exchange: request → accept → complete → credits move | BROKEN | **Named cause: ASP.NET cannot settle an exchange and cannot until `exchanges` gains per-party confirmation columns (an EF migration).** Driven 2026-08-21 through the unchanged React UI, both arms. **Laravel control passes the whole journey and the credits move**: smoke step `journey-exchange-request-accept-complete` created exchange 63 from a listing page, the provider accepted → started → marked complete, both parties confirmed, and the balances moved requester 99→98 / provider 26→27 (exactly −1/+1) with the settlement visible in wallet history. **ASP.NET SKIPPED at the first gate** on the shared dev container, which was still running the pre-fix image. Four defects on the path were FIXED this batch: (a) `GET /v2/exchanges/config` was a do-nothing stub returning four fields no client reads and omitting all four the client does — which took the whole feature off the map, because a missing boolean reads as `false` and every exchange page then renders its own "workflow not enabled" state at HTTP 200; (b) the client sends POST for accept/decline/start/complete while the real controller declared only PUT, so accept landed on an `AdminOnly` empty-array stub and answered the listing owner 403 "Admin access required"; (c) three guard-free aliases mutated status with no participant and no transition check — `POST /confirm` was measured live reopening a COMPLETED exchange that already carried a settled `TransactionId`, and it set the wrong status while doing it; (d) `POST /v2/exchanges/{id}/rate` returned HTTP 500 `AmbiguousMatchException` and `GET /v2/exchanges/{id}/ratings` was a stub shadowing the real read, whose replacement also had to be re-nested as `data:{ratings,has_rated}` because the client reads one level deeper than the first fix returned. Condition 5 now clean for all 14 endpoints on the path; stub baseline lowered 317→316. Guarded by `ExchangeJourneyContractTests` (14 tests, 14 passing) plus the smoke step. **Unmet conditions: 1 and 2.** 1 — the fixed build was never deployed to the shared dev container (three sessions shared it this batch), so nothing here is proved through the browser against ASP.NET; 2 — credits do not move and cannot. Next actions, in order: Schema agent adds `requester_confirmed_at/hours`, `provider_confirmed_at/hours`, `final_hours`; Implementer builds two-party settlement on `PersonalWalletLedgerService` the way `GroupExchangeService.CompleteAsync` already does; then re-run the control arm with the new image |
