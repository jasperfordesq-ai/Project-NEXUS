...
| ---: | --- | --- | --- |
| 1.1 | Sign-up: register form → "verify your email" | CERTIFIED | Comprehensive same-run artifact `react-smoke-2026-08-24T21-52-39-523Z.json`: unchanged React registration reached its success and verification-email state on both arms; `journey-sign-up` MATCH. |
...
| 1.12 | Feed filter tabs filter | CERTIFIED | Same artifact: `journey-feed-filters` MATCH; Listings hid the same-run post and Posts restored it on both arms. |
| 1.13 | Feed: create post | CERTIFIED | Same artifact: `action-create-feed-post` MATCH; the unchanged post composer issued a successful mutation and the exact post survived a fresh chronological-feed navigation. |
| 1.14 | Feed: comment, visible after | CERTIFIED | Same artifact: `journey-feed-comment` MATCH; the UI issued the comment mutation and a server-loaded comment survived a fresh feed navigation on both arms. |
...
| 1.17 | Listings browse | CERTIFIED | Same artifact: `journey-listings-browse` MATCH; each default catalogue contained exactly one card and detail link for the listing created and persisted earlier in that arm. |
...
| 1.22 | Events browse + discovery filters | CERTIFIED | Same artifact: `journey-events-browse-filters` MATCH; unique-title search isolated the persisted event, Past hid it, Upcoming restored it, and its detail link remained correct. |
| 1.23 | Event: RSVP | CERTIFIED | Same artifact: `action-rsvp-event` MATCH; a real UI mutation changed RSVP state and the chosen state survived reload on both arms. |
...
| 1.26 | Messages: send into existing thread | CERTIFIED | Same artifact: `journey-message-send` MATCH; the exact UI-sent message remained in the existing conversation after reload on both arms. |
| 1.27 | Messages: start a new conversation | CERTIFIED | **CERTIFIED locally 2026-08-25; banking awaits the next batched push and green CI.** Same-run artifact `aspnet-backend/artifacts/smoke/react-smoke-2026-08-25T12-50-10-080Z.json`: `journey-message-new-conversation` = MATCH (8/8 MATCH overall, no failures or skips). The focused paired React batch starts with no direct relationship row, searches and selects a known same-tenant recipient through New Message, sends a unique stamped body, reloads the sender thread, then signs in as the recipient in a fresh browser context and opens the exact stamped inbox thread. Focused v2 integration coverage independently proves the recipient's fresh inbox and partner-keyed thread contain that body and the correct sender, while established tenant-isolation tests refuse unrelated tenants. Implementation `f4c75e169`. |
| 1.28 | Messages: voice + attachment send and play back | OPEN | ASP.NET now projects persisted voice rows with `is_voice`, duration and the participant-authorized `/api/v2/messages/{id}/voice` URL, and ordinary attachments use `/api/v2/messages/{message}/attachments/{attachment}` rather than owner-only generic file links. Focused recipient-reload and byte-stream regressions pass (17/17 across voice, direct-message and private-media suites). Certification remains blocked before either backend is called: the unchanged React attachment preview renders and exposes an enabled native `button[type=submit]`, but automated click and Enter probes emit zero native submit events and no POST. React changes are outside this ASP.NET-only workstream, so the paired send/playback journey remains unproven and the score is unchanged. |
...
| 1.30 | Wallet: history renders | CERTIFIED | Same artifact: `journey-wallet-history` MATCH; the ledger contained the exact same-run transfer note and amount after reload (ASP.NET 0.25 credit, Laravel 1 credit). |
| 1.31 | Members directory browse | CERTIFIED | Same artifact: `journey-members-connect` browse arm MATCH; each directory exposed the known other member's exact identity and real profile link. Connection mutation remains row 1.32. |
| 1.32 | Connections: request → accept → appears both sides | CERTIFIED | **CERTIFIED locally 2026-08-25; banking awaits the next batched push and green CI.** Same-run artifact `aspnet-backend/artifacts/smoke/react-smoke-2026-08-25T12-50-10-080Z.json`: `journey-connections-request-accept` = MATCH (8/8 MATCH overall, no failures or skips). The focused paired React batch resets only the two disposable actors, requests from the real profile Connect control, accepts as the recipient, then freshly reloads My Connections for each member and requires the intended other identity. Two focused integration regressions first went RED on React's `pending_received` contract and now prove request/accept, consumed row shape, both accepted lists and unrelated-tenant non-disclosure/non-acceptance. Implementation `f4c75e169`. |
...
| 1.34 | Settings: edit a field and save it | CERTIFIED | **CERTIFIED locally 2026-08-25; banking awaits the next batched push and green CI.** Same-run artifact `aspnet-backend/artifacts/smoke/react-smoke-2026-08-25T12-50-10-080Z.json`: `journey-settings-profile-persist` = MATCH (8/8 MATCH overall, no failures or skips). The focused paired React batch edits Tagline through Settings, requires the real PUT, signs in through a fresh browser context, requires the unique value, then restores the original through the same UI and proves the restoration after reload. No fixture or direct database write supplies the effect. Implementation `f4c75e169`. |
...
| 3.33 | Caring Community: my relationships and trust tier (`/my-relationships`, `/my-trust-tier`, `/caregiver/link`) | OPEN | 🔴 A relationship record is never authorisation. `can_view_messages` on `account_relationships` is deliberately **not** enforced in Laravel — do not wire it up in .NET or present it as working. 🔴 **PARTIAL EVIDENCE 2026-08-29 — deliberately still OPEN.** The `/caregiver/link` third of this row now has a committed paired instrument, `e2e/journeys/caring/caregiver-parity.spec.ts` (run with `e2e/playwright.caring.config.ts`), which drives the caregiver-CONSENT lifecycle against both backends **in one execution** and asserts effects by reading each engine's own database — MariaDB for Laravel, PostgreSQL for ASP.NET. Six tests pass: identical outcomes at every gate (create→pending, recipient confirm keeps it pending, approve→active, the 422 refusals for premature approval / missing attestation / reasonless rejection, active-only schedule and on-behalf authority, and the same notification types to both participants), plus every field the React components read present on ASP.NET. ADR-0004 conditions 1, 2, 3, 4 and 5 are met **for that journey**; condition 5 was checked by reading `CaregiverSupportService` rather than inferred. 🔴 **The ROW stays OPEN because it is wider than the journey proved:** `/my-relationships` and `/my-trust-tier` were not exercised at all, and the STAFF caregiver-review queue built alongside this work is named by no row in this ledger — whether that needs a Tier 5 reserve row is an owner decision, not a bookkeeping one, and is unresolved. Do not mark this row CERTIFIED on the strength of the caregiver-consent evidence alone. 🔴 One real ASP.NET defect was found and fixed by the paired run: `RemoveLinkAsync` answered **500** (Npgsql 23505 against `ccl_tenant_caregiver_recipient_status_unique`) where Laravel answered 204, and the link stayed ACTIVE — a caregiver could not end a relationship they had ended once before, while the authority it confers remained in force. Both engines carry that index; Laravel deletes the superseded inactive row first and ASP.NET did not. Regression-guarded by `CaringCommunityCaregiverControllerUnitTests.RemoveLink_WhenAnInactiveLinkForThePairAlreadyExists_StillDeactivates`. Scope origin: 2026-08-21 R5 expansion |
...
| 4.2 | Signed-out `/` (choose a community) vs Laravel control | RENDERS | pair compared. The fixture asymmetry once diagnosed here (`GET /api/v2/tenants` excludes the master tenant unless `include_master=1`, and the disposable Laravel fixture held *only* the master tenant, listing 0 against ASP.NET's 4) was **FIXED 2026-08-21**: `parity-fixture.sql:128-180` seeds two non-master tenants (950001 Globex Neighbourhood Network, 950002 Riverside Timebank). The matching `KNOWN_DIFFERENCES` entry was removed 2026-08-28 per that list's shrink-only contract; the next instrument run compares `/` for real |
