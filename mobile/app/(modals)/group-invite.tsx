...
function GroupInviteScreen() {
  ...
  return <ModalErrorBoundary><SafeAreaView className="flex-1 bg-background" style={{ flex: 1, backgroundColor: theme.bg }}><AppTopBar title={t('groups:invite_accept.title')} backLabel={t('common:back')} fallbackHref={'/(modals)/groups' as Href} /><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 16, paddingBottom: 40 }}>
    {!validToken ? <EmptyState icon="warning-outline" title={t('groups:invite_accept.title')} subtitle={t('groups:invite_accept.error_invalid')} /> : previewState.isLoading && !preview ? <LoadingSpinner /> : isRefusalStatus(previewState.errorStatus) ? <EmptyState icon="warning-outline" title={t('groups:invite_accept.title')} subtitle={t('groups:invite_accept.error_invalid')} testID="group-invite-refused" /> : previewState.error || !preview ? <EmptyState icon="warning-outline" title={t('groups:invite_accept.title')} subtitle={previewState.error ?? t('groups:invite_accept.error_invalid')} actionLabel={t('common:buttons.retry')} onAction={previewState.refresh} /> : <HeroCard className="rounded-panel"><HeroCard.Body className="items-center gap-4 p-6"><Text accessibilityRole="header" className="text-center text-2xl font-bold" style={{ color: theme.text }}>{acceptedGroupId ? t('groups:invite_accept.success_title') : preview.group.name}</Text><Chip size="sm" variant="secondary"><Chip.Label>{t('groups:members', { count: preview.group.member_count })}</Chip.Label></Chip><Text className="text-center text-base leading-6" style={{ color: theme.textSecondary }}>{acceptedGroupId || alreadyMember ? t('groups:invite_accept.success_description') : t('groups:invite_accept.description')}</Text>{actionError ? <Text accessibilityRole="alert" className="text-center text-sm text-danger">{actionError}</Text> : null}<HeroButton className="w-full" isDisabled={accepting} onPress={() => acceptedGroupId ? openGroup(acceptedGroupId) : alreadyMember ? openGroup(preview.group.id) : void accept()}><HeroButton.Label>{acceptedGroupId || alreadyMember ? t('groups:invite_accept.go_to_group') : accepting ? t('groups:invite_accept.accepting') : t('groups:invite_accept.accept')}</HeroButton.Label></HeroButton></HeroCard.Body></HeroCard>}
  </ScrollView></SafeAreaView></ModalErrorBoundary>;
}

export default withRouteGate(GroupInviteScreen, 'group-invite');
