            ...
            <HeroCard className="rounded-panel">
              <HeroCard.Body className="gap-4 p-5">
                ...
                <View className="items-center gap-2 border-b border-default-100 pb-5">
                  ...
                  <Text className={`text-sm font-semibold ${receipt.data.status === 'completed' ? 'text-success' : receipt.data.status === 'pending' ? 'text-muted-foreground' : 'text-danger'}`}>{t(`donations.status.${receipt.data.status}`)}</Text>
                </View>
