...
function IdeationOutcomesScreen() {
  ...
  return <ModalErrorBoundary><SafeAreaView className="flex-1 bg-background" style={{ flex: 1, backgroundColor: theme.bg }}>
    ...
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} refreshControl={<RefreshControl refreshing={dashboardState.isLoading && Boolean(dashboard)} onRefresh={dashboardState.refresh} tintColor={primary} colors={[primary]} />}>
      <RefreshFailedNotice error={dashboard ? dashboardState.error : null} onRetry={dashboardState.refresh} />
      {dashboardState.isLoading && !dashboard ? <LoadingSpinner /> : !dashboard ? <EmptyState icon="warning-outline" title={t('ideation:challenges.load_error')} subtitle={dashboardState.error ?? undefined} actionLabel={t('ideation:actions.retry')} onAction={dashboardState.refresh} /> : <View className="gap-4">
