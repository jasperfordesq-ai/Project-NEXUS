...
function IdeationCampaignDetailScreen() {
  ...
  return <ModalErrorBoundary><SafeAreaView className="flex-1 bg-background" style={{ flex: 1, backgroundColor: theme.bg }}>
    ...
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} refreshControl={<RefreshControl refreshing={campaignState.isLoading && Boolean(campaign)} onRefresh={campaignState.refresh} tintColor={primary} colors={[primary]} />}>
      ...
      {campaignState.isLoading && !campaign ? <LoadingSpinner /> : !campaign ? <EmptyState icon="warning-outline" title={t('ideation:challenges.load_error')} subtitle={campaignState.error ?? undefined} actionLabel={t('ideation:actions.retry')} onAction={campaignState.refresh} /> : <View className="gap-4">
