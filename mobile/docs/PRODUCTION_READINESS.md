...
| 9 | Visual correctness | **Weak** | Contrast gated (37 assertions, found 7 real failures). 3 screens pixel-gated — 🔴 but the gate was BLIND to light-grey-on-white change until 2026-08-19 (reported 40px for a real 9,168px change); tolerance corrected and guarded, and it immediately caught a second changed screen. Visual sweep: 93 images over 31 screens (light, dark, 1.3x text). Three bugs found and fixed — an invisible icon on every listing card, content scrolling under the clock app-wide, and a clipped filter row. One systemic finding open: two clashing brand colours in 128 controls, awaiting an owner decision. See [VISUAL_AUDIT.md](VISUAL_AUDIT.md). ~106 screens still unphotographed |
