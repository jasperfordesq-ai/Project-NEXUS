...
describe('mobile Irish gamification challenges and journeys', () => {
  it('does not fall back to English except for reviewed progress displays', () => {
    ...
    const reviewedPaths = [...englishFlat.keys()].filter((path) =>
      path.startsWith('challenges.') || path.startsWith('journeys.'),
    );
