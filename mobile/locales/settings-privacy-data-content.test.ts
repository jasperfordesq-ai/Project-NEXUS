...
describe('mobile Irish settings privacy and data tools', () => {
  it('uses Irish except for the JSON format name', () => {
    const englishFlat = flatten(english);
    const irishFlat = flatten(irish);
    const reviewedPaths = [...englishFlat.keys()].filter((path) =>
      path.startsWith('blockedUsers.') || path.startsWith('dataExport.'),
    );

    for (const path of reviewedPaths) {
      expect(irishFlat.get(path)).toBeDefined();
      if (path === 'dataExport.format.json') {
