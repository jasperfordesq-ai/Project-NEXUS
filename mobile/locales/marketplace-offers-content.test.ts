...
describe('mobile Irish marketplace offers', () => {
  it('contains reviewed Irish with only the numeric amount example unchanged', () => {
    const englishFlat = flatten(english);
    const irishFlat = flatten(irish);
    const reviewedPaths = [...englishFlat.keys()].filter((path) => path.startsWith('offers.'));

    for (const path of reviewedPaths) {
      expect(irishFlat.get(path)).toBeDefined();
      if (path === 'offers.amountPlaceholder') {
        expect(irishFlat.get(path)).toBe(englishFlat.get(path));
      } else {
        expect(irishFlat.get(path)).not.toBe(englishFlat.get(path));
      }
    }
    expect(irishFlat.get('offers.sendCounter')).toContain('friththairiscint');
    expect(irishFlat.get('offers.acceptCounter')).not.toContain('gcuntar');
  });
});
