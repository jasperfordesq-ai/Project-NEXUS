...
export type CreationDraftKind = 'goal' | 'poll' | 'message' | 'quiz-attempt' | 'event-communication' | 'event-people' | 'event-session-registration' | 'event-registration-settings' | 'event-registration-form' | 'event-guest-attendance' | 'event-invitation-campaign' | 'event-retention' | 'event-invitation-revoke' | 'event-agenda' | 'event-staff' | 'event-safety';
...
const usesDraftFile = (scope: CreationDraftScope) => (scope.kind === 'event-registration-form' || scope.kind === 'event-invitation-campaign' || scope.kind === 'event-agenda' || scope.kind === 'event-safety') && Platform.OS !== 'web';
