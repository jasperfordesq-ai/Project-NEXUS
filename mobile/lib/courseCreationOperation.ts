...
export interface CourseCreationOperation { storageKey: string; key: string; createdAt: number }
const reservations = new Map<string, Promise<CourseCreationOperation>>();
...
export async function reserveCourseCreationOperation(intent: string): Promise<CourseCreationOperation> {
  const [user, tenant] = await Promise.all([
    storage.getJson<{ id: number }>(STORAGE_KEYS.USER_DATA),
    storage.get(STORAGE_KEYS.TENANT_SLUG),
  ]);
  if (!user?.id || !tenant) throw new Error('Course creation identity unavailable');
  const forOriginalIdentity = async (pending: Promise<CourseCreationOperation>): Promise<CourseCreationOperation> => {
    ...
    if (currentUser?.id !== user.id || currentTenant !== tenant) {
      throw new Error('Course creation identity changed before save');
    }
    return operation;
  };

  const hash = await digestStringAsync(CryptoDigestAlgorithm.SHA256, JSON.stringify([tenant, user.id, intent]));
  const storageKey = `nexus_course_creation_${hash}`;
  ...
  const reservation = withStorage(storageKey, async () => {
    ...
    if (raw) {
      const saved = JSON.parse(raw) as CourseCreationOperation & { completed?: boolean };
    ...
    const operation = { storageKey, key: mutationIdempotencyKey('mobile-course-create'), createdAt: Date.now() };
...
export async function completeCourseCreationOperation(operation: CourseCreationOperation): Promise<void> {
  ...
  await withStorage(operation.storageKey, async () => {
    try {
      ...
      if (!raw || (JSON.parse(raw) as CourseCreationOperation).key !== operation.key) {
