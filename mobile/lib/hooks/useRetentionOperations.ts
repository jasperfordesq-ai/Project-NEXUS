...
import type { RetentionIntent } from '../api/eventRegistration';
import { executeRetentionOperation, recoverRetentionOperation,
  loadRetentionOperation, type RetentionScope, type SavedRetentionOperation } from '../eventRetentionOperation';

type Receipt = Awaited<ReturnType<typeof executeRetentionOperation>>;
type State = { epoch: number; saved: SavedRetentionOperation | null; ready: boolean; storageFailed: boolean;
...
export function useRetentionOperations(scope: RetentionScope, permitted: boolean, active: boolean, onAccepted: (receipt: Receipt) => void) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const saved = await loadRetentionOperation({ tenantId, userId, eventId });
  ...
  async function perform(kind: 'submit' | 'recover', intent?: RetentionIntent) {
    ...
    try {
      ...
      const receipt = kind === 'submit' ? await executeRetentionOperation(owner, intent!, isCurrent)
          : await recoverRetentionOperation(owner, isCurrent);
  ...
  return { ...visible, reload, submit: (intent: RetentionIntent) => perform('submit', intent),
