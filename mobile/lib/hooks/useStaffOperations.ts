...
import { executeStaffOperation, recoverStaffOperation,
  loadStaffOperation, discardRejectedStaffOperation, type StaffOperationScope, type StaffOperationIntent, type SavedStaffOperation } from '../eventStaffOperation';

type Receipt = Awaited<ReturnType<typeof executeStaffOperation>>;
type State = { epoch: number; saved: SavedStaffOperation | null; ready: boolean; storageFailed: boolean;
...
export function useStaffOperations(scope: StaffOperationScope, permitted: boolean, active: boolean, onAccepted: (receipt: Receipt) => void) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const saved = await loadStaffOperation({ tenantId, userId, eventId });
  ...
  async function perform(kind: 'submit' | 'recover' | 'discard', intent?: StaffOperationIntent) {
    ...
    try {
      ...
      if (kind === 'discard') {
        await discardRejectedStaffOperation(owner, visible.saved!.key, isCurrent);
        return;
      }
      const receipt = kind === 'submit' ? await executeStaffOperation(owner, intent!, isCurrent)
          : await recoverStaffOperation(owner, isCurrent);
  ...
  return { ...visible, reload, submit: (intent: StaffOperationIntent) => perform('submit', intent),
