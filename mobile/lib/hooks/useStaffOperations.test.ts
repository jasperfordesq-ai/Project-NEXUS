...
import { loadStaffOperation as load, executeStaffOperation as execute, recoverStaffOperation as recover, discardRejectedStaffOperation as discard, type SavedStaffOperation } from '../eventStaffOperation';
jest.mock('../eventStaffOperation', () => ({ loadStaffOperation: jest.fn(), executeStaffOperation: jest.fn(), recoverStaffOperation: jest.fn(), discardRejectedStaffOperation: jest.fn() }));
const scope = { tenantId: 2, userId: 3, eventId: 7 };
const intent = { action: 'revoke' as const, assignmentId: 8 };
const pending: SavedStaffOperation = { ...scope, schemaVersion: 1, status: 'pending', key: 'saved-key', intent, attempts: 1 };
beforeEach(() => {
  jest.clearAllMocks();
...
it('loads pending work without resending, blocks new changes and permits explicit recovery', async () => {
  ...
  const { result } = renderHook(() => useStaffOperations(scope, true, true, accepted));
...
it.each(['account', 'community', 'event', 'permission', 'background'] as const)('ignores a late receipt and old callbacks after %s changes', async change => {
  const accepted = jest.fn();
  const { result, rerender } = renderHook((props: { scope: typeof scope; permitted: boolean; active: boolean }) => useStaffOperations(props.scope, props.permitted, props.active, accepted), {
...
it('does not submit when durable storage cannot be read', async () => {
  ...
  const { result } = renderHook(() => useStaffOperations(scope, true, true, jest.fn()));
...
it('blocks callbacks retained after unmount', async () => {
  const { result, unmount } = renderHook(() => useStaffOperations(scope, true, true, jest.fn()));
  await waitFor(() => expect(result.current.ready).toBe(true));
