...
import { executeInvitationCampaignOperation, recoverInvitationCampaignOperation,
  loadInvitationCampaignOperation, type InvitationCampaignScope, type SavedInvitationCampaignOperation } from '../eventInvitationCampaignOperation';
...
export function useInvitationCampaignOperations(scope: InvitationCampaignScope, permitted: boolean, active: boolean, onAccepted: (receipt: Receipt) => void) {
  ...
  async function perform(kind: 'submit' | 'recover', intent?: InvitationCampaignIntent) {
