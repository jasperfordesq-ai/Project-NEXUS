...
export interface GroupFileUploadOperation { storageKey: string; key: string; createdAt: number }
const reservations = new Map<string, Promise<GroupFileUploadOperation>>();
...
export async function reserveGroupFileUploadOperation(intent: string): Promise<GroupFileUploadOperation> {
  const [user, tenant] = await Promise.all([
    storage.getJson<{ id: number }>(STORAGE_KEYS.USER_DATA),
    storage.get(STORAGE_KEYS.TENANT_SLUG),
  ]);
  if (!user?.id || !tenant) throw new Error('Group file upload identity unavailable');

  const forOriginalIdentity = async (
    pending: Promise<GroupFileUploadOperation>,
  ): Promise<GroupFileUploadOperation> => {
    ...
    if (currentUser?.id !== user.id || currentTenant !== tenant) {
      throw new Error('Group file upload identity changed before save');
    }
    return operation;
  };

  const hash = await digestStringAsync(CryptoDigestAlgorithm.SHA256, JSON.stringify([tenant, user.id, intent]));
  const storageKey = `nexus_group_file_upload_${hash}`;
  ...
  const reservation = withStorage(storageKey, async () => {
    ...
    if (raw) {
      const saved = JSON.parse(raw) as GroupFileUploadOperation & { completed?: boolean };
    ...
    const operation = {
      storageKey,
      key: mutationIdempotencyKey('mobile-group-file-upload'),
      createdAt: Date.now(),
    };
...
export async function completeGroupFileUploadOperation(operation: GroupFileUploadOperation): Promise<void> {
  ...
  await withStorage(operation.storageKey, async () => {
    try {
      ...
      if (!raw || (JSON.parse(raw) as GroupFileUploadOperation).key !== operation.key) {
