...
export interface VolunteerOpportunityCreationOperation { storageKey: string; key: string; createdAt: number }

const reservations = new Map<string, Promise<VolunteerOpportunityCreationOperation>>();
...
export async function reserveVolunteerOpportunityCreationOperation(intent: string): Promise<VolunteerOpportunityCreationOperation> {
  ...
  const reservation = withStorage(storageKey, async () => {
    ...
    if (raw) {
      const saved = JSON.parse(raw) as VolunteerOpportunityCreationOperation & { completed?: boolean };
    ...
    const operation = { storageKey, key: mutationIdempotencyKey('mobile-volunteer-opportunity'), createdAt: Date.now() };
...
export async function completeVolunteerOpportunityCreationOperation(operation: VolunteerOpportunityCreationOperation): Promise<void> {
  ...
  await withStorage(operation.storageKey, async () => {
    try {
      ...
      if (!raw || (JSON.parse(raw) as VolunteerOpportunityCreationOperation).key !== operation.key) {
