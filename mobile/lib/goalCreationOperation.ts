...
export interface GoalCreationOperation { storageKey: string; key: string; createdAt: number }
const reservations = new Map<string, Promise<GoalCreationOperation>>();
...
export async function reserveGoalCreationOperation(intent: string): Promise<GoalCreationOperation> {
  const [user, tenant] = await Promise.all([
    storage.getJson<{ id: number }>(STORAGE_KEYS.USER_DATA), storage.get(STORAGE_KEYS.TENANT_SLUG),
  ]);
  if (!user?.id || !tenant) throw new Error('Goal creation identity unavailable');
  const forOriginalIdentity = async (pending: Promise<GoalCreationOperation>): Promise<GoalCreationOperation> => {
    const operation = await pending;
    const [currentUser, currentTenant] = await Promise.all([
      storage.getJson<{ id: number }>(STORAGE_KEYS.USER_DATA), storage.get(STORAGE_KEYS.TENANT_SLUG),
    ]);
    if (currentUser?.id !== user.id || currentTenant !== tenant) throw new Error('Goal creation identity changed before save');
    return operation;
  };

  const hash = await digestStringAsync(CryptoDigestAlgorithm.SHA256, JSON.stringify([tenant, user.id, intent]));
  const storageKey = `nexus_goal_creation_${hash}`;
  ...
  const reservation = withStorage(storageKey, async () => {
    ...
    if (raw) {
      const saved = JSON.parse(raw) as GoalCreationOperation & { completed?: boolean };
    ...
    const operation = { storageKey, key: mutationIdempotencyKey('mobile-goal-create'), createdAt: Date.now() };
...
export async function completeGoalCreationOperation(operation: GoalCreationOperation): Promise<void> {
  ...
  await withStorage(operation.storageKey, async () => {
    try {
      ...
      if (!raw || (JSON.parse(raw) as GoalCreationOperation).key !== operation.key) {
