...
🔴 **The ASP.NET edition is PAUSED (owner decision, 2026-09-11) — not abandoned;
it resumes when the rest of the platform is more mature.** Until then Laravel is
the only backend this frontend targets. Do not spend time on ASP.NET parity, the
certification ledger, `dev:dotnet`, or the API-call inventory's ASP.NET side. See
[`ADR-0005`](../aspnet-backend/docs/decisions/ADR-0005-aspnet-edition-halted.md).
What survives from the dual-backend era is the neutrality rule, which is good
engineering regardless: **no backend-specific branch in a page or component**
(`if (backend === 'aspnet')` and the like), and the API client expresses Laravel's
contract. The paragraphs below are kept for history; `check:backend-guardrails`
still enforces the neutrality rule.
