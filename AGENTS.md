...
- **Laravel is the production/default backend contract, and ASP.NET is a committed second edition.** ASP.NET work must make ASP.NET conform to the Laravel API rather than changing frontend behaviour — never fix a mismatch in a frontend, never add an ASP.NET-specific branch, never invent a success response.
