...
**Platform Features:** Multi-tenant architecture, gamification (badges, XP, challenges), federation, real-time WebSockets (Pusher), push notifications, light/dark theme and PWA. The sole native mobile app is the Expo / React Native client in `mobile/`, for both Android and iOS. The former web-wrapper mobile project is abandoned and its integration has been removed. Do not restore it or treat it as a supported client.
...
| [docs/REACT-DUAL-BACKEND.md](docs/REACT-DUAL-BACKEND.md) | React backend-neutrality guardrails (historical dual-backend doc; ASP.NET edition halted 2026-09-11) |
| [aspnet-backend/docs/decisions/ADR-0005-aspnet-edition-halted.md](aspnet-backend/docs/decisions/ADR-0005-aspnet-edition-halted.md) | 🔴 Why the ASP.NET edition is halted, what was switched off, and what would re-open it |
...
- 🔴 **The ASP.NET edition is PAUSED (owner decision, 2026-09-11) — paused, not abandoned: development resumes when the rest of the platform is more mature.** Until then Laravel is the only backend being worked on. Do not read, test, build, fix or plan work in `aspnet-backend/`; do not spend session or CI time on it. Its four CI jobs in `platform-contracts.yml` are pinned `if: false`, its local dev containers (`nexus-aspnet-dev-*`) are stopped, and the reasons and the resume steps are in [ADR-0005](aspnet-backend/docs/decisions/ADR-0005-aspnet-edition-halted.md). The code, ledger and ADRs stay in the repository untouched so resuming is a switch, not a rediscovery; nothing below this line about ASP.NET describes current work.

  *Historical framing, kept so the record stays honest:* until 2026-09-11 the bullet read "Laravel is the production/default backend contract, and ASP.NET is a committed second edition. ASP.NET work must make ASP.NET conform to the Laravel API rather than changing frontend behaviour — never fix a mismatch in a frontend, never add an ASP.NET-specific branch, never invent a success response." The frontend-neutrality rule in that sentence still holds on its own merits: never add a backend-specific branch to a frontend.

  🔴 **Corrected 2026-08-21.** This bullet described ASP.NET as "development-only" and "an optional future alternative, not Laravel's planned replacement". That framing was wrong and it cost the workstream weeks — an optional project gets no scoped delivery plan, so its goal stayed at the maximal reading and no agent was authorised to narrow it. **The ASP.NET edition is a committed deliverable with a commercial driver: a segment of public-sector buyers require a .NET stack as a condition of procurement.** Laravel remains the production default *until the ASP.NET edition is certified*. Growth still never triggers a cutover.
...
**Preferred: gated deploy from the dev machine.** Run `bash scripts/deploy.sh` — it runs the larastan/PHPStan static-analysis gate first (catches the job-offers class of bug; only NEW findings beyond `phpstan-baseline.neon` block — override a false alarm with `ALLOW_PHPSTAN_FAIL=1`), pushes, **then blocks until GitHub confirms the pushed commit is fully checked**, runs the blue/green deploy below, and finally **watches production error rates for 30 minutes after the traffic switch** and, once the server's detached post-deploy render has published (`scripts/wait-for-prerender-publish.sh`, bounded), **probes every host as a crawler** (`scripts/check-prerender-delivery.mjs`, non-blocking but loud — until 2026-09-16 it fired seconds after the switch and cried BLANK on snapshots the render had not yet replaced) (`scripts/postdeploy-watch.mjs` — Sentry counts scoped to the new release tag; on a spike it alarms and prints the rollback command, never rolls back itself; skip with `SKIP_POSTDEPLOY_WATCH=1`). The watch needs a Sentry token with `org:read, project:read, event:read` in `.secrets.local/sentry.env` as `SENTRY_AUTH_TOKEN_MONITOR`; without one it reports UNVERIFIED rather than pretending health. The static gate runs locally in the `nexus-php-app` container, so it adds only a couple of minutes and can't break the server-side deploy. The raw steps below still work as a fallback — but they bypass the check gate and the watch.
...
- 🔴 **The verifier watches TWO workflows (since 2026-08-17).** `ci.yml` in full, and `platform-contracts.yml` **only for its `Web UK …` jobs** — because `web-uk` is the production accessible frontend serving three live hostnames, yet its suite/lint/brand/accessibility jobs live there, not in ci.yml. The ci.yml job that used to cover the accessible frontend (`Accessible Frontend Release Gate`) was deleted with Blade on 2026-08-14 and never replaced, so for three days a deploy could ship the accessible frontend unverified. Consequences to preserve: a new `Web UK …` job must be added to `REQUIRED_JOBS` too; every **non**-`Web UK` job in that workflow is deliberately ignored so the ASP.NET track can never gate a Laravel release (the reason is release isolation between two editions, **not** that ASP.NET is paused or optional — it is neither; the exclusion itself stays); and both `Web UK` jobs must keep their `workflow_dispatch`/`schedule` escapes, or `--trigger` can never clear a refusal and deploys block forever. `scripts/predeploy-ci-check.sh` dispatches both workflows for the same reason.
- The nightly `schedule` run (03:30 UTC) has its **own concurrency group** so it and push runs never cancel each other — keep that when editing `concurrency:`. 🔴 `platform-contracts.yml` sweeps **weekly** (Mondays 04:10 UTC), not nightly. That is fine for the `Web UK` jobs because they also run on every push that touches `web-uk/**`, and path-based inheritance covers the rest — but after a long stretch with no web-uk change, the first deploy may have to force a run.
...
ASP.NET backend and Web UK frontend, live at
