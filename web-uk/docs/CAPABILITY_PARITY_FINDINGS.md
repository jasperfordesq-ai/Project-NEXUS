...
| 1.4 | ✅ **FIXED 2026-08-17.** `applies_to` is now sent on create only, where `all_listings` is the genuine default; an update omits it and leaves the stored scope untouched. The form offers no scope field, so it must not overwrite one it never showed the seller. 🔴 **Two pre-existing tests asserted the widening PUT** and were corrected. *Original finding:* editing any coupon reset its discount to the seller's whole catalogue. | `marketplace-actions.js:290` hardcoded `applies_to: 'all_listings'` in the shared create/update payload; `MerchantCouponService.php:100-109` re-normalises to it |
...
| 1.6 | ✅ **FIXED 2026-08-17.** The POST now reads the exchange's `split_type` first and, on a weighted exchange, sets `weight` to the hours the organiser typed — which is what weighted means: when a role's requests sum to `total_hours` everyone gets exactly what was entered, otherwise shares scale proportionally. `equal` and `custom` never read weight, so they still send 1. No new locale keys were needed. 🔴 **Still open, deliberately:** web-uk cannot *create* a weighted split (no `split_weighted` string exists in any of the eleven catalogues) and the detail page does not name the split type the way React does. Both need one new key across eleven files. *Original finding:* a hardcoded `weight: 1` discarded the typed hours AND moved every existing participant's share, because adding a weight changes the divisor. | `group-exchange-actions.js:124` hardcoded `weight: 1`; `:107` coerces to `equal`; `GroupExchangeService.php:416-431` divides by role weight |
| 1.7 | ✅ **FIXED 2026-08-17.** Complete is no longer rendered on a disputed exchange. Cancel deliberately stays — cancelling a disputed exchange is a legitimate organiser action, which is why `disputed` was not simply folded into `isClosed`. *Original finding:* with every confirmation in place, completing a disputed exchange MOVED THE CREDITS; React requires `pending_confirmation`. | `group-exchanges.js:232` treated only completed/cancelled as closed; `GroupExchangeService.php:823-829` likewise |
| 1.8 | ✅ **FIXED 2026-08-17.** A `community_delivery` listing now sends `shipping_method: 'community_delivery'`, so the order can receive a delivery offer instead of storing NULL behind a success message. | `marketplace-actions.js:661-678`; `MarketplaceOrderService.php:843-854` → NULL |
| 1.9 | ✅ **FIXED 2026-08-17.** A failed member donation now returns to `/wallet/manage` with the member option still selected, the search context preserved and the amount, message and chosen recipient echoed back; a community-fund failure still returns to `/wallet`, where that form lives. The recipient is pinned to the id the member actually chose rather than the first search result, so a reordered result set cannot pay a different person. *Original finding:* every failure landed on the community-fund form with the amount pre-filled, so pressing Donate again gave the credits to the fund. | `wallet.js:404` → `views/wallet/index.njk:52` hardcodes `target=community_fund`; `manage.njk:84-87` omitted the `recipient_q` field `index.njk:74` includes |
| 1.10 | ✅ **FIXED 2026-08-17.** Three faults with one visible symptom. (a) There were **two** price-label implementations — `priceLabel` handled a cash-or-credits listing and `cardPriceLabel` did not — so every card in every list showed the credits alone, hiding the cash option, then the price changed on click-through. The card variants are deleted; both template fields now come from the one function, so they cannot drift again. `search.njk` also hardcoded the credits-blue tag colour and now follows the price type. (b) `formatCredits` concatenated **"time credits" in English**, and the hybrid label went through a translator pinned to English at module load, so both read English in all eleven languages. A new `request-translator.js` reads the request locale already carried in AsyncLocalStorage — no threading through call sites — and the three other modules holding an English-pinned translator were checked and are correct (they use it as a fallback behind `req.t`, which is its proper purpose). (c) Hardcoded English `'Free'` now uses the existing translated `common.free`. One new key, `common.credits_price`, was added to all eleven Laravel catalogues and derived from each locale's **already-reviewed** `hybrid_price` wording (every locale orders it `:credits <unit>`), so no machine translation was involved. 🔴 **A pre-existing test asserted `not.toContain('GBP 12.50 or 3 time credits')`** — it pinned the bug as correct — and was corrected. 🟡 **Raised, not changed:** German `common.free` is `"Frei"`, which is free-as-in-unoccupied; a free-of-charge item would more naturally be `"Kostenlos"` or `"Gratis"`. It is a reviewed value used elsewhere, so it is flagged here rather than edited silently. | `marketplace.js:498-512`, `:474`, `:503`; the buyer's method choice is `MarketplaceOrderService.php:358-380` — `price` **or** `time_credit_price × quantity`, never both |
| 1.11 | ✅ **FIXED 2026-08-17.** A `POST /group-exchanges/{id}/start` route and a Start section now exist, gated exactly as React gates it: organiser, status `draft` or `pending_participants`, and at least one person in each role. When a role is empty the reason is stated instead of the button silently vanishing. Fixing it exposed **three more faults in the same workflow**, all corrected here because Start is useless without them. (a) `editable` listed `['draft','pending','approved']` — **`pending` and `approved` are not statuses**; the real enum is `draft, pending_participants, pending_broker, active, pending_confirmation, completed, cancelled, disputed`. So an exchange genuinely sitting in `pending_participants` could be neither edited nor started. The invented values are gone. (b) Confirm rendered on a **draft**, asking people to confirm something not under way, and their confirmation could not lead anywhere because Complete also needs a started exchange; Confirm and Complete now both require `pending_confirmation`, as in React. Cancel stays available in every open state, including disputed. (c) Four of the eight statuses had **no translated label** and fell through to an English `headline()` in all eleven languages; three now reuse the reviewed wording already written for one-to-one exchanges, so both exchange types describe the same state identically, and `pending_participants` was added. Six new keys were hand-written into all eleven `govuk_alpha.php` catalogues — that file has a zero-untranslated gate, so no locale carries the English value. 🔴 **Two pre-existing tests asserted Complete and Confirm on a draft** and were corrected. | no `/start` in `group-exchange-actions.js`; the endpoint has existed all along at `routes/api.php:3587`; `GroupExchangeService.php:648` is the only `notifyParticipantsToConfirm` caller |
| 1.12 | ✅ **FIXED 2026-08-17.** Each tab now maps to the statuses that actually exist: Active asks for `pending_payment,paid,shipped`, Completed for `completed,delivered`, Cancelled for `cancelled,refunded`. 🔴 **A pre-existing test asserted the verbatim tab name** and was corrected. *Original finding:* the buyer's Active tab was permanently empty; Completed hid every `delivered` order and Cancelled every `refunded` one. | `marketplace.js:197` sent `status=active`, absent from the enum; `MarketplaceOrderService.php:1367-1373` exact-matches |
