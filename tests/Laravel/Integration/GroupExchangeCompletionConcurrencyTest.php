...
{
    public function test_completion_revalidates_after_a_concurrent_participant_removal(): void
    {
        ...
        try {
            ...
            DB::purge();
            foreach (['remover', 'completer'] as $role) {
                ...
                if ($pid === 0) {
                    ...
                    try {
                        ...
                        $announced = false;
                        DB::connection()->beforeExecuting(function (string $query) use ($role, &$announced, $sockets): void {
                            $sql = strtolower($query);
                            if (!$announced && $role === 'remover' && str_starts_with($sql, 'delete from `group_exchange_participants`')) {
                                $announced = true; fwrite($sockets[1], "ready\n");
                                if (trim((string) fgets($sockets[1])) !== 'go') throw new \RuntimeException('Removal barrier timed out');
                            }
                        ...
                        $result = $role === 'remover' ? ['removed' => $service->removeParticipant($exchangeId, $users[1])] : $service->complete($exchangeId);
            ...
            fwrite($workers['remover']['socket'], "go\n");
            $results = [];
            ...
            self::assertTrue($results['remover']['removed']);
            self::assertFalse($results['completer']['success'], 'Settlement must not credit a removed provider from an old split');
            DB::reconnect();
