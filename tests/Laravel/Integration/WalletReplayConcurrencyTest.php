...
final class WalletReplayConcurrencyTest extends TestCase
{
    ...
    public function test_unlike_wallet_operations_from_one_sender_serialize_without_losing_either_movement(): void
    {
        foreach (['pcntl_fork', 'pcntl_waitpid', 'stream_socket_pair', 'posix_kill'] as $function) {
            if (!function_exists($function)) {
                self::markTestSkipped("{$function} is required for concurrent wallet verification.");
        ...
        $workers = [];
        try {
            foreach ([20, 5, 5] as $balance) {
                $users[] = (int) DB::table('users')->insertGetId([
                    'tenant_id' => $this->testTenantId, 'name' => 'Wallet unlike-operation fixture',
                    'email' => 'wallet-unlike-' . bin2hex(random_bytes(10)) . '@example.test',
                    'password_hash' => password_hash(bin2hex(random_bytes(16)), PASSWORD_BCRYPT),
                    'balance' => $balance, 'role' => 'member', 'status' => 'active',
                    'is_active' => true, 'is_approved' => true, 'preferred_language' => 'en',
                    'created_at' => now(), 'updated_at' => now(),
            ...
            DB::purge();
            foreach (['transfer', 'donation'] as $kind) {
                $sockets = stream_socket_pair(STREAM_PF_UNIX, STREAM_SOCK_STREAM, STREAM_IPPROTO_IP);
                self::assertNotFalse($sockets);
                $pid = pcntl_fork();
                self::assertNotSame(-1, $pid);
                    ...
                    try {
                        ...
                        DB::connection()->beforeExecuting(function ($query) use ($sockets, &$waiting): void {
                            if ($waiting && str_contains($query, '`users`') && str_contains($query, 'for update')) {
                                ...
                                fwrite($sockets[1], "ready\n");
                                if (trim((string) fgets($sockets[1])) !== 'go') {
                                    throw new \RuntimeException('Unlike-operation barrier timed out');
                                }
                            }
                        });

                        $result = $kind === 'transfer'
                            ? app(WalletService::class)->transfer($users[0], [
                                'recipient' => $users[1], 'amount' => 2,
                                'description' => 'Concurrent transfer',
                                'idempotency_key' => 'unlike-transfer',
                            ])
                            : app(CreditDonationService::class)->donateToMember(
                                $users[0], $users[2], 2, 'Concurrent donation', sha1('unlike-donation')
                            );
                        if ($kind === 'donation' && empty($result['success'])) {
                            throw new \RuntimeException($result['message'] ?? $result['error']);
                        }
                        fwrite($sockets[1], json_encode(['success' => true], JSON_THROW_ON_ERROR) . "\n");
                        fclose($sockets[1]);
                        exit(0);
                    } catch (\Throwable $error) {
                        fwrite($sockets[1], json_encode(['error' => $error->getMessage()]) . "\n");
            ...
            DB::reconnect();
            self::assertEquals(16, DB::table('users')->where('id', $users[0])->value('balance'));
            self::assertEquals(7, DB::table('users')->where('id', $users[1])->value('balance'));
            self::assertEquals(7, DB::table('users')->where('id', $users[2])->value('balance'));
            self::assertSame(2, DB::table('transactions')->where('sender_id', $users[0])->count());
            self::assertSame(1, DB::table('wallet_transfer_receipts')->where('sender_id', $users[0])->count());
            self::assertSame(1, DB::table('credit_donations')->where('donor_id', $users[0])->count());
        } finally {
            foreach ($workers as $worker) {
