...
abstract class AccessSweepTestCase extends TestCase
{
    ...
    // ================================================================
    // Multi-parameter routes (person and child identifiers) — moved verbatim
    // from CrossCommunityAccessSweepTest on 2026-09-11 so the same-community
    // sweep can reuse them. Only `private` became `protected`.
    // ================================================================
    /**
     ...
     */
    protected function multiParamChildEndpoints(): array
    {
        $endpoints = [];
     ...
     * @return array{0:?string,1:?string}
     */
    protected function resolveChildParam(string $path, string $param): array
    {
     ...
     * @return array{0:string,1:?string}
     */
    protected function fillFromIds(string $uri, array $ids): array
    {
        $index = 0;
        $missing = null;
     ...
     * refused.
     */
    protected function seedControlRelationships(User $controlPerson, User $actor, array $ownIds): void
    {
     ...
     * @return list<array<string,mixed>>
     */
    protected function multiParamPersonEndpoints(): array
    {
        $endpoints = [];
     ...
     */
    protected function fillMultiParamUri(array $plan, string $uri, array $ownIds, int $personId): array
    {
        $missing = null;
        $index = 0;
     ...
     */
    protected function personReferenceColumns(): array
    {
     ...
     */
    protected function personReferenceFingerprints(array $refs, int $personId): array
    {
        $fingerprints = [];
        $unreadable = 0;
    ...
    protected const GHOST_ID = 2000000000;
     ...
     */
    protected function ghostComparison(string $method, string $path, array $requestBody, int $foreignStatus, string $foreignBody): array
    {
        try {
