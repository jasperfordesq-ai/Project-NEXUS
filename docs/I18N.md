...
paths in both translation helpers handle it like any other locale — do not skip
...
2. Add the key to every other locale file, then **translate it**. Adding it with
   ...
   warning above. Use the translation helpers:
   - React JSON — `node scripts/translate-i18n-gaps.mjs --google`
   - PHP `lang/*.php` — `node scripts/translate-php-lang-gaps.mjs --google --namespace <file>.php`
     (a separate script; the React one deliberately excludes PHP)
