...
| [SECURITY-ASSURANCE.md](SECURITY-ASSURANCE.md) | How security assessment works: route-table-wide sweeps, the finding register and its status vocabulary, evidence handling, and what a customer with a supplier-assurance obligation can ask for. |
...
| [DEPLOYMENT-LESSONS.md](DEPLOYMENT-LESSONS.md) | What went wrong the first time the platform was stood up on a new server: the hardcoded production values in deployment config, the caching layers that hide a working fix, Plesk and Azure traps, and why a server-side check passing does not mean the application works. Read before building a new environment. |
