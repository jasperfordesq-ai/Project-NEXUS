...
class MessageService
{
     ...
     *
     * Unread message state lives in TWO stores and both have to move together:
     * `messages.is_read`, and the bell rows in `notifications` that
     * NotifyMessageReceived writes with link `/messages/{senderId}`. Only the
     * first was cleared here until 2026-09-02, so the second accumulated for
     * ever — and because `/v2/notifications/counts` reports its `messages`
     * category from those rows, a member who had read every message still met a
     * non-zero count at the next login. That stale count was what the native
     * app's message badge rendered.
     */
    public static function markAsRead(int $partnerId, int $userId): int
    {
        ...
        self::markConversationNotificationsRead($partnerId, $userId);

        return $marked;
    }

    /**
     * Clear the bell rows generated by one conversation.
     *
     * Scoped to the recipient's own `user_id` plus the conversation's link, and
     * deliberately NOT to a tenant id: these are the acting member's own rows,
     * `user_id` is already unique platform-wide, and an extra tenant predicate
     * here can only turn a legitimate clear into a silent no-op.
     *
     * `federation_message` is excluded on purpose — the message write above
     * skips federated rows (`is_federated = 0`), so their bell entries are not
     * this method's to clear.
     */
    private static function markConversationNotificationsRead(int $partnerId, int $userId): void
    {
        try {
            DB::table('notifications')
                ->where('user_id', $userId)
                ->where('is_read', 0)
                ->whereIn('type', ['message', 'new_message', 'message_received'])
                ->where('link', '/messages/' . $partnerId)
                ->update(['is_read' => 1, 'read_at' => now()]);
        } catch (\Throwable $e) {
            // The message rows are already read; a failure to tidy the bell must
            // not fail the member's request. Logged rather than swallowed so the
            // badge-returns symptom stays diagnosable instead of looking like a
            // fresh bug.
            Log::error('MessageService::markConversationNotificationsRead failed', [
                'user_id' => $userId,
