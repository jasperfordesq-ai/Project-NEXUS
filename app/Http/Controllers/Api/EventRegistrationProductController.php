...
final class EventRegistrationProductController extends BaseApiController
{
    ...
    public function retentionHistory(int $id): JsonResponse
    {
        ...
        return $this->execute(fn (): array => $this->queries->retentionHistory($id, $this->actor(), $page, $perPage));
    }
    ...
    private function productError(EventRegistrationFoundationException $exception): JsonResponse
    {
        ...
        if ($reason === 'event_registration_form_revision_conflict') {
            return $this->respondWithError('EVENT_REGISTRATION_CONFLICT', __('api.invalid_input'), 'expected_form_revision', 409);
        }
        ...
        if ($reason === 'event_invitation_revocation_invalid') {
            // Exact-key replay is resolved first; this refusal did not revoke the invitation.
            return $this->respondWithError('EVENT_REGISTRATION_VALIDATION_FAILED', __('api.validation_failed'), 'invitation_state', 422);
        }
