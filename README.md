...
> **Version 1.8.0 — Generally Available** — Project NEXUS V1.8.0 is generally available and in active production use. Production runs a Laravel 12 + PHP 8.2+ API behind three clients: the React 19 web app, a GOV.UK-styled accessible site on Node 22 / Express, and an Expo / React Native mobile app. It is currently in use by communities in **Ireland** and being evaluated by communities in the **United Kingdom**, **Spain**, **Switzerland**, and the **United States**. Newer modules may still ship with their own per-module maturity label (Beta / Preview). Contributions and feedback are welcome.
...
This is **version 1.8.0 — generally available**, in active production use. Per-module maturity (GA / Beta / Preview) is published on the in-app `/features` page and the public Changelog:
